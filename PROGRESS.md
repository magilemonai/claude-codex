...
<!-- record · claudecodex-20260719-101053 · 2026-07-19-claudecodex-claudecodex-sortie-selfcheck-boot · M5 · 2026-07-19T14:19:14.635Z -->
## 2026-07-19T14:19:14.635Z — M5: THE POST ITSELF (split by hand — the dots and the memory follow). The Meridian POST types in before the existing boot art in bootSequence (story.js ~829): short log lines — the MOTD, VERA's handshake coming online, a 'mounting reality/ … legacy naming convention' wink. Any key or click completes it instantly (reuse skipTyping/skipRequested, engine.js ~289-297); cues via snd.* only; prefers-reduced-motion shows all lines at once (matchMedia branch). It plays every boot for now — the first-boot memory is the next mission. NO index.html changes in this mission. Append checks.mjs assertions: skip guard present in the POST loop, reduced-motion branch present, no createOscillator outside snd.

- id: `claudecodex-20260719-101053`
- order: THE POST ITSELF (split by hand — the dots and the memory follow). The Meridian POST types in before the existing boot art in bootSequence (story.js ~829): short log lines — the MOTD, VERA's handshake coming online, a 'mounting reality/ … legacy naming convention' wink. Any key or click completes it instantly (reuse skipTyping/skipRequested, engine.js ~289-297); cues via snd.* only; prefers-reduced-motion shows all lines at once (matchMedia branch). It plays every boot for now — the first-boot memory is the next mission. NO index.html changes in this mission. Append checks.mjs assertions: skip guard present in the POST loop, reduced-motion branch present, no createOscillator outside snd.
- model: `claude-opus-4-8`
- merged: 2026-07-19T14:19:14.635Z → main
...
<!-- record · claudecodex-20260719-101914 · 2026-07-19-claudecodex-claudecodex-sortie-selfcheck-boot · M6 · 2026-07-19T14:32:24.184Z -->
## 2026-07-19T14:32:24.184Z — M6: THE DOTS AND THE MEMORY (the split's other half). Titlebar dots (index.html ~120-122) start unlit under a boot body class and light r→y→g in sequence as the POST plays (CSS in index.html; the CRT palette untouched). First-boot memory: a codex_meta field (loadMeta, engine.js ~52) shortens or skips the POST on repeat visits — first boot is the show. Append the remaining checks.mjs assertions: the boot body class exists, the meta gate is read before the POST.

- id: `claudecodex-20260719-101914`
- order: THE DOTS AND THE MEMORY (the split's other half). Titlebar dots (index.html ~120-122) start unlit under a boot body class and light r→y→g in sequence as the POST plays (CSS in index.html; the CRT palette untouched). First-boot memory: a codex_meta field (loadMeta, engine.js ~52) shortens or skips the POST on repeat visits — first boot is the show. Append the remaining checks.mjs assertions: the boot body class exists, the meta gate is read before the POST.
- model: `claude-opus-4-8`
- merged: 2026-07-19T14:32:24.184Z → main
...
<!-- record · claudecodex-20260719-103255 · single · 2026-07-19T14:50:52.979Z -->
## 2026-07-19T14:50:52.979Z — THE MERCY RULING (Cody's call 2026-07-19: fix it the Act-V way — the M4 crew's own hand-trace names the model). Make Acts II and III ticket gates idempotent under replay, the way chapter5():319 already recomputes G.frags from flags: a DECIDED ticket never re-queues on resume — chapter2()'s guard (~story.js:1045) and the T-2107 setTimeout (~:1073), and chapter3()'s unconditional re-adds (finale.js:24-25), each consult the persisted decision flags before adding; a re-queued-then-decided ticket must not increment G.mercy a second time (key mercy on the per-ticket decision, not the act of deciding). NO save-schema change — the flags already persisted by M4 are the source of truth (the crew's trace lists them per gate). Extend checks.mjs: a static assertion that each ticket add-site sits behind its decision guard. Verify every line number by grep first — the M4 crew's DISPATCH.md hand-trace (in the plan's history) is the map.

- id: `claudecodex-20260719-103255`
- order: THE MERCY RULING (Cody's call 2026-07-19: fix it the Act-V way — the M4 crew's own hand-trace names the model). Make Acts II and III ticket gates idempotent under replay, the way chapter5():319 already recomputes G.frags from flags: a DECIDED ticket never re-queues on resume — chapter2()'s guard (~story.js:1045) and the T-2107 setTimeout (~:1073), and chapter3()'s unconditional re-adds (finale.js:24-25), each consult the persisted decision flags before adding; a re-queued-then-decided ticket must not increment G.mercy a second time (key mercy on the per-ticket decision, not the act of deciding). NO save-schema change — the flags already persisted by M4 are the source of truth (the crew's trace lists them per gate). Extend checks.mjs: a static assertion that each ticket add-site sits behind its decision guard. Verify every line number by grep first — the M4 crew's DISPATCH.md hand-trace (in the plan's history) is the map.
- model: `— not recorded`
- merged: 2026-07-19T14:50:52.979Z → main
